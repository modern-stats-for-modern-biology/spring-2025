...
If I proposed a different DAG where `StressCondition` alters `Sociability` not only directly but also indirectly by altering neuronal activity (measured by the `CeLcfos` variable) in the amygdala (a likely scenario), then including `Sociability` as a control variable/covariate in the regression model would be wrong because (`Sociability` being a collider in this alternative DAG) the statistical model would introduce a spurious correlation that confounds the estimate of the **direct** causal effect of `StressCondition` on `CeLcfos`.
...
Here I use the `modelsummary()` and the `compare_performance()` functions from the [`modelsummary`](https://modelsummary.com/) and [`performance`](https://easystats.github.io/performance/) packages, respectively, to evaluate and compare models.
...
Statistical models contain a lot more insights than what you get from simply looking at the parameter estimates. In many cases, like models with multiple interactions, non-linear effects, non-standard families, complex random effect structures, the parameters can be hard if not impossible to interpret as a quantity of interest/estimand.

Here I use the `estimate_means()` and `estimate_contrasts()` functions from the [`modelbased`](https://easystats.github.io/modelbased/) package (a wrapper around the more powerful and flexible hence more difficult to learn [`marginaleffects`](https://marginaleffects.com/) and [`emmeans`](https://rvlenth.github.io/emmeans/) packages) to compute and plot predictions, estimate quantities of interest, and perform hypothesis tests with these quantities. The [`marginaleffects`](https://marginaleffects.com/) website is also a great learning resource about how to interpret statistical models. Another good resource is this [blog post](https://www.andrewheiss.com/blog/2022/05/20/marginalia/).
