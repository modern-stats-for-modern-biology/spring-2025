...
This notebook shows how to use various packages to 1) import data from an Excel spreadsheet ([`readxl`](https://readxl.tidyverse.org/)), 2) evaluate ([`performance`](https://easystats.github.io/performance/)) and compare ([`modelsummary`](https://modelsummary.com/)) models, and 3) estimate marginal effects ([`ggeffects`](https://strengejacke.github.io/ggeffects/)) using the best fitting model (`m5`) of the mouse stress/sociability/neuronal activation experiment illustrated during the lecture of week 10 (2025-03-24, **Chapter 9: Models with interactions**) by [Evan Schaffer](https://www.schafferlab.com/evan-schaffer).
...
d <- cbind(id,soc,cel) %>%
  ...
  ungroup() %>% 
  mutate(logCeLcfos = log(CeLcfos), # for EDA, not modeling!
         Sex = factor(Sex),
...
```{r}
favstats(~ log(CeLcfos), data = d)
```

```{r}
gf_histogram(~ log(CeLcfos), data = d)
```

```{r}
gf_qq(~ log(CeLcfos), data = d) %>% gf_qqline()
```
...
```{r}
plot_predictions(m5, condition = c("Sociability", "StressCondition"))
```
...
```{r}
predictions(m5, newdata = datagrid(StressCondition = unique), transform = exp) %>% tidy()
```
```{r}
emmeans(m5, ~ StressCondition | Sociability, type = "response")
```

```{r}
predictions(m5, newdata = datagrid(StressCondition = unique), transform = exp) %>% hypotheses(difference ~ reference) %>% tidy()
```

```{r}
emmeans(m5, ~ StressCondition | Sociability, type = "response") %>% contrast("trt.vs.ctrl")
```

```{r}
predictions(m5, newdata = datagrid(StressCondition = unique, Sociability = fivenum), transform = exp) %>% tidy()
```
...
```{r}
predictions(m5, newdata = datagrid(Sociability = fivenum), transform = exp) %>% tidy()
```


```{r}
soc_mean <- mean(d$Sociability, na.rm = TRUE)
soc_sd <- sd(d$Sociability, na.rm = TRUE)

emmeans(m5, ~ StressCondition | Sociability, at = list(Sociability = c(soc_mean - soc_sd, soc_mean, soc_mean + soc_sd)), type = "response") %>% pairs()
```

```{r}
predictions(m5, newdata = datagrid(Sociability = fivenum), transform = exp) %>% hypotheses(difference ~ sequential) %>% tidy()
```

```{r}
predictions(m5, newdata = datagrid(Sociability = fivenum, StressCondition = unique), transform = exp) %>% tidy()
```
