...
```{r}
estimate_means(m5, by = c('Sociability', 'StressCondition')) %>% plot()
```

```{r}
estimate_means(m5, by = c('StressCondition', 'Sociability'))
```

## Marginal effects and hypothesis tests

```{r}
estimate_contrasts(m5, contrast = 'StressCondition')
```

```{r}
estimate_contrasts(m5, contrast = 'StressCondition', by = 'Sociability')
```
