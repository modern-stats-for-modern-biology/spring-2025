...
At this level of `Sociability`, the direct causal effect of `StressCondition` on `CeLcfos` is not statistically significant, but since we modeled `Sociability` as a moderator of this effect, the estimated fold-change of `CeLcfos` between the `Shock` and `Control` groups (the direct causal effect of `StressCondition` on `CeLcfos`) may be different at different levels of `Sociability`:
...
```{r}
estimate_contrasts(m5, contrast = 'StressCondition', by = 'Sociability', transform = exp, p_adjust = "bonferroni")
```
