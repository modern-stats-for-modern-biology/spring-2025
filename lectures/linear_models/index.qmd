...
## &hellip; and the model says: ["*line*"!]{.fragment} [obs ≠ predictions]{.fragment}
...
df %>%
  ...
  gf_labs(title = TeX(r'(Monte Carlo simulation of $y_i = \beta_0 + \beta_1 x_i + \epsilon_i\; \beta_0 = 45, \beta_1 = 0.6, \sigma = 4$)'), y = "Height (cm, mean-centered)", x = "Weight (Kg)") %>%
...
-   The `stan_glm` function uses the [Stan](https://mc-stan.org/) probabilistic programming language to fit the model
...
-   [`brms`](https://paulbuerkner.com/brms/) is a similar package that provides a more flexible and powerful `brms` function to fit Bayesian general (and generalized) linear models specified using the [R formula syntax](https://www.econometrics.blog/post/the-r-formula-cheatsheet/)


## Fit the model to the data

::: nonincremental
-   The Bayesian estimate is the joint posterior **probability** distribution of the parameters $\beta_0$ (intercept), $\beta_1$ (slope), and $\sigma$ (error spread) given the data
:::
...
::: nonincremental
-   The Bayesian estimate is the joint posterior **probability** distribution of the parameters $\beta_0$ (intercept), $\beta_1$ (slope), and $\sigma$ given the data
:::
...
::: nonincremental
-   The frequentist estimate is the maximum **likelihood** point estimate and confidence interval of the parameters $\beta_0$ (intercept) and $\beta_1$ (slope) given the data
:::
