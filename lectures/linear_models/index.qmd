...
-   The next step is to use an estimator to estimate the parameters $\beta_0$, $\beta_1$, and $\sigma$ that best fit the data $\{ (x_i, y_i) \}$

-   Simulated data (predictions) can also be generated from this model by additionally specifying the parameters $\beta_0$, $\beta_1$, and $\sigma$
...
df %>%
  ...
  gf_point(H ~ W, data = df %>% group_by(H) %>% sample_n(3), color = "skyblue4", size = 1) %>%
