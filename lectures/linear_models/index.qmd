...
subtitle: "Data analysis workflow using linear models"
date: 2025-03-03
...
The **estimand** is the target of statistical inference (unknown quantity)
...
Fit the *statistical model* to the *data* using an **estimator** (i.e., an inference algorithm) in order to obtain an **estimate** of the *estimand*
...
5.  **Choose** estimator and **test** (e.g., with simulated data, sensitivity analysis, prior predictive checks), [given model, data, estimand, question]{style="color: orangered"}

6.  **Fit** the model to the data using an estimator to obtain an estimate of the estimand, [given estimator, model, data, estimand, question]{style="color: orangered"}

7.  **Evaluate** the estimate/fitted model with the training data (e.g., cross-validation, residual analysis, posterior predictive checks), [given estimate, estimator, model, data, estimand, question]{style="color: orangered"}
...
-   **Neyman–Pearson hypothesis testing**
    ...
    -   In neuroscience, studies suffer from low power (8–31% [@Button2013]), which undermines the reliability of these decisions and inflates both Type I and Type II error rates
...
[Fisher RA 1926](https://doi.org/10.23637/rothamsted.8v61q)
:::
:::::
...
[![](images/dosso_dossi-allegory_of_fortune.jpg){fig-align="center" height="440"}](https://www.getty.edu/art/collection/object/103RJG)
...
"*caused by shrimp scampi*" 😱 unless effect were caused by 🦐 indigestion!
...
of the causal linear association between height (H) and weight (W) in adult humans using a general **linear model** (LM)[; using LM instead of GLM in order to avoid confusion with the broader class of **generalized linear models** (GLM)]{.fragment}
...
-   With $(x, y)$ data, the goal of statistical inference is often to estimate the association between *variates* ($y$) and *covariates* ($x$) [$\quad \quad p(y \mid M) \ne p(y \mid x, M)$]{.fragment}
...
of the causal linear association between height (H) and weight (W) in adult humans using a general linear model (LM)

-   $\text{WEIGHT (W)} = \text{HEIGHT (H)} + \text{OTHER STUFF (U)}$

-   $y_i = \beta_0 + \beta_1 x_i + \epsilon_i \quad \quad \epsilon_i \stackrel{\text{i.i.d.}} \sim \text{Norm}(0, \sigma)$

-   $y_i \stackrel{\text{i.i.d.}} \sim \text{Norm}(\mu_i, \sigma) \quad \quad \mu_i = \beta_0 + \beta_1 x_i$
...
df %>%
  ...
  gf_labs(title = TeX(r'(Monte Carlo simulation of $y_i = \beta_0 + \beta_1 x_i + \epsilon_i\; \beta_0 = 45, \beta_1 = 0.6, \sigma = 4$)')) %>%
...
::: {.column width="40%"}
$$
y_i \stackrel{\text{i.i.d.}} \sim \text{Norm}(\mu_i, \sigma) \\[.5em]
\mu_i = \beta_0 + \beta_1 x_i \\[.5em]
\beta_0 \sim \text{Norm}(160,30) \\[.5em]
\beta_1 \sim \text{Unif}(0,1) \\[.5em]
...
bayes.lm.fit <- stan_glm(weight ~ height_c, data = d, family = gaussian())
```
...
Frequentist estimate is the maximum **likelihood** point estimate and confidence interval of the parameters $\beta_0$ (intercept) and $\beta_1$ (slope), and $\sigma$ given the data
...
## Compare the Bayesian and frequentist estimates

```{r}
modelsummary::msummary(list("bayes" = bayes.lm.fit, "mle" = mle.lm.fit))
```
...
```{r}
#| fig-height: 4
performance::check_model(mle.lm.fit, check = c("linearity", "normality"))
```
:::
...
```{r}
#| fig-height: 4
performance::check_model(mle.lm.fit, check = c("homogeneity"))
```
...
## Evaluate the fitted model with the training data
