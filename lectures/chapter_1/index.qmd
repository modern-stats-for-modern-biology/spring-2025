...
-   In this course, we will use three languages—**R**, **logic**, and **mathematics**—to illustrate and apply probability theory and statistical inference
...
-   The `print` function is called by default in R and therefore it can be omitted
...
```{=html}
<iframe src="https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/Binomial" width="100%" height="100%"></iframe>
```
...
-   If any argument without a default value remains unmatched, then R throws an error!
...
::: nonincremental
-   An environment is a collection of objects (data and functions) stored in the computer’s memory, each with a unique name
:::

-   Objects that you name in your code are stored in the so called **global environment**
...
-   Use the built-in `install.packages` function to download and install one or more packages from the [CRAN](https://cran.r-project.org/) online repository or from local files
...
::: nonincremental
-   Use the `suppressPackageStartupMessages` function to suppress messages during the loading of a package
:::
...
::: nonincremental
-   Once loaded, the objects (functions and data) contained in a package are accessible within the homonymous environment
:::
...
::: nonincremental
-   You can explicitly specify the package using the :: operator to access a function from a particular package, avoiding ambiguity
:::
...
::: nonincremental
-   Use the `?` operator or the `help` function to get the documentation for an object
:::
...
# help for all functions and datasets in the `mosaic` package
help(package = "mosaic") 
