...
-   A valid name must start with a letter or a dot
...
-   A function takes 0, 1, or more input objects (**arguments**) and returns 1 output object
...
```{r}
#| output-location: default
add_up_two_nums <- function(num1, num2) {
  out <- num1 + num2
  return(out)
}
```
...
-   The `print` function stores code that, when called, sends its first argument (named `x`) to an output device (screen, printer, file, etc.) and returns it invisibly as its output object
...
-   It’s useful to distinguish between the formal arguments of a function, and the arguments supplied when calling a function

-   When calling a function, supplied arguments are matched with the formal arguments first by exact name (perfect matching), then by partial name (prefix matching), and finally by position (if unnamed or name does not match)

-   When calling a function, I'd recommend that you:

    -   Use positional matching only for the first one or two commonly used arguments to enhance readability

    -   Avoid using prefix matching

    -   Supply named arguments after unnamed arguments

-   If a function includes the special "match all the unmatched" `...` argument, any formal argument that follow `...` can only be supplied using its exact name

-   Arguments can have default values, allowing them to be omitted in function calls

-   If arguments without default values remain unmatched, R will throw an error 
...
::: fragment
```{r}
dbinom(size = 10, x = 3, prob = 0.5)
```
:::

::: fragment
```{r}
dbinom(size = 10, 3, TRUE, prob = 0.5)
```
:::
...
-   Or you can store the intermediate values as objects to be passed from fn to fn:
...
::: nonincremental
-   Or you can use the pipe operator `%>%` provided by the `magrittr` package and automatically loaded by the `dplyr` package:
:::
...
-   Since version 4.1.0, R has a native pipe operator `|>` whose behaviour is by and large the same as that of `%>%`, but with [some subtle but crucial differences](https://www.tidyverse.org/blog/2023/04/base-vs-magrittr-pipe)!
...
::: fragment
```{r}
assign("x", 17.2) # assigns the value `17.2` to the object named `x` and invisibly returns `NULL`
