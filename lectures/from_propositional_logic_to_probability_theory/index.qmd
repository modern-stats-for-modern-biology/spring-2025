...
-   **Propositional logic** is a mathematical language that we use to:
    ...
    -   **represent knowledge** with propositions, and
    -   **reason** about the **truth** of propositions, given the knowledge base

-   **Probability theory** extends propositional logic to **reason** about the **plausibility** of propositions, given the knowledge base
...
-   Axioms of probability theory
    ...
    -   [Principle of equivalence](https://en.wikipedia.org/wiki/Principle_of_indifference): Equal ignorance implies equal probability

::: notes
A longer definition of the principle of indifference (also called the principle of insufficient reason):

“When there is no available evidence or rational basis to favor one possibility over another, all possibilities should be assigned equal probability. That is, if a set of mutually exclusive and exhaustive outcomes are indistinguishable given the known information, they must be treated as equally probable.”
...
-   Probability measures how strongly the premises entail the conclusion

-   Probability measures the inductive strength of an argument

-   Probability measures the plausibility of the conclusion, given the premises
...
-   For example, in neuroscience, it provides a foundation for modeling and understanding how the brain works

-   In particular, Bayes’ theorem is central to [active inference models](https://en.wikipedia.org/wiki/Free_energy_principle), where the brain is viewed as a Bayesian engine that continuously updates its internal representation of external reality based on new sensory evidence

-   In this way, the same equation that underpins statistical inference also helps explain perception, learning, and decision-making in the brain

-   Bayes’ theorem is undoubtedly one of the most revolutionary ideas in human history!
...
-   If events $A$ and $B$ are independent, then the product rule simplifies to:
...
::: nonincremental
-   Two events (or propositions) $A$ and $B$ are **mutually exclusive** iff they cannot occur simultaneously (or they cannot both be true), i.e., their intersection is the empty set ($\emptyset$)
:::

::: fragment
$$
P(A \land B) = P(\emptyset) = 0
$$
:::

-   If events $A$ and $B$ are mutually exclusive, then the sum rule simplifies to:
...
::: nonincremental
-   Two events (or propositions) $A$ and $B$ are **exhaustive** iff at least one of them must occur (or be true), i.e., their union is the sample space ($\Omega$)
:::
...
-   If events $A$ and $B$ are both mutually exclusive, i.e., $P(A \land B) = 0$, and exhaustive, then the sum rule simplifies to:
...
-   **Mutually exclusive and exhaustive events** are a.k.a. **partitions** of the sample space



## Probability theory: Exhaustive events

![](proportional_syllogism_on_steroids/Slide6.png){fig-align="center" height="50"}
...
P(A) + P(B) = 1 \quad \text{iff A and B are both mutually exclusive and exhaustive, i.e.,}\ P(A \lor B) = 1
\end{array}
$$
:::
...
-   Suppose we define:
    ...
    -   $D$: The event that a woman has breast cancer
    -   $T$: The event that a diagnostic test for breast cancer is positive

-   Given (known quantities/premises/evidence):
...
-   Wanted (unknown quantity/conclusion/hypothesis):

    -   **Positive predictive value:** $\; P(D \mid T)$\
...
::: fragment
$$
P(D \mid T) = \frac{P(D) \cdot P(T \mid D)}{P(T)} = \frac{P(D) \cdot P(T \mid D)}{P(T \land D) + P(T \land \lnot D)} = \frac{P(D) \cdot P(T \mid D)}{P(D) \cdot P(T \mid D) + P(\lnot D) \cdot P(T \mid \lnot D)}
$$
:::
...
-   This example highlights the critical role of the base rate (prevalence) in interpreting diagnostic tests for diseases with low prevalence

-   Failure to take the base rate (prior probability) into account is known (and hopefully taught in medical schools across the world) as the **base rate fallacy**
...
::: nonincremental
-   Two events (or propositions) $A$ and $B$ are **exhaustive** iff at least one of them must occur (or be true)
:::
...
::: nonincremental
-   Moreover, if events $A$ and $B$ are both mutually exclusive and exhaustive, then:
:::
...
-   Let $\{X_1, X_2, \ldots, X_n\}$ be a set of mutually exclusive and exhaustive events (i.e., a partition of the sample space).
...
-   Since events $B$ and $\lnot B$ are mutually exclusive and exhaustive, we have:
