...
-   If a function includes the special "*match all the unmatched arguments*" `...` argument, any formal argument that follow `...` can only be supplied using its exact name
...
-   Use the `install` function from the `BiocManager` package to install one or more packages from CRAN, [Bioconductor](https://bioconductor.org/), and other online repositories (e.g., [GitHub](https://github.com/))
...
-   As of today, CRAN and Bioconductor feature [21,939](https://www.r-pkg.org/) and [2,289](https://www.bioconductor.org/packages/release/BiocViews.html) packages, respectively!

-   Browse for packages using [CRAN](https://cloud.r-project.org/web/views/) and [Bioconductor](https://www.bioconductor.org/packages/release/BiocViews.html) "*task views*", or search for packages using [rdrr.io](https://rdrr.io/) and [Rdocumentation](https://www.rdocumentation.org/)
...
-   When you combine elements of different types into an atomic vector, they are coerced in this order: `character` → `double` → `integer` → `logical`
...
-   Most mathematical operators and functions (`+`, `-`, `*`, `sum`, `mean`, `min`, `max`, etc.) will automatically coerce logical values to numeric values: `TRUE` becomes `1` and `FALSE` becomes `0`

::: fragment
```{r}
as.integer(c(TRUE, TRUE, TRUE, FALSE))
```
:::
...
::: fragment
| Logic (R) | Arithmetic (R) | Probability (math) |
|----|----|----|
| `!A` | `1 - A` | $P(\text{not-}A) = 1 - P(A)$ |
| `A & B` | `A * B` | $P(A \text{ and } B) = P(A) \cdot P(B)$ |
| `A | B` | `A + B - (A * B)` | $P(A \text{ or } B) = P(A) + P(B) - P(A) \cdot P(B)$ |
:::
...
``` r
as.numeric("17.2")  # Converts to 123 (numeric)
as.character(17.2)  # Converts to "123" (character)
as.logical(0)      # Converts to FALSE
```

-   Be cautious of coercion errors!
...
-   Factors in R store categorical data and may require conversion before numerical operations.

``` r
f <- factor(c("low", "medium", "high"))
as.character(f)  # Converts to "low" "medium" "high"
as.numeric(f)    # Converts to underlying integer levels
```

-   Understand automatic coercion behavior when mixing types.

-   Use explicit coercion functions to avoid unexpected results.

-   Handle factors carefully to avoid unintended numerical interpretations.
...
-   Data frames are the most common way tabular data (i.e., "*row x column*" data, like an Excel spreadsheet) are stored in R
...
-   Use the `data.frame` or `tibble` function to create a data frame or tibble, respectively
...
-   This data frame is "*tidy*":
    ...
    -   Each row represents a **observation** (person; it is good practice to use one dataframe for each type/class of observation unit)
...
-   It's generally good practice to "*keep things tidy*":
    ...
    -   Tidy data frames are easier to work with using tools from the [`tidyverse`](https://www.tidyverse.org/)
...
-   Use the `str` or `glimpse` function to look at the structure of a data frame
...
-   `dplyr` is a package with functions (a.k.a. "*verbs*") that form a consistent grammar of data transformation:
    ...
    -   `pull` pulls a variable/column based on its name
...
-   These verbs can be combined with the `group_by` function to perform operations "*by group*"
