...
::: fragment
```{r}
df[c(FALSE, TRUE, TRUE, TRUE)] # remove 1st column (select 2nd, 3rd and 4th column)
```
:::
...
-   `dplyr` is a package with functions (a.k.a. "*verbs*") that form a consistent grammar of data transformation:
    ...
    -   `select` selects columns/variables based on their names
    -   `pull` pulls a column/variable based on its name
    -   `filter` selects rows/observations based on their values
    -   `mutate` adds new columns/variables that are functions of existing columns/variables
    -   `arrange` changes the ordering of the rows/observations
