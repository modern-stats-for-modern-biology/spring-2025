...
-   Data frames are lists of vectors (usually atomic vectors, but also lists) of equal length
-   Each element of a data frame (an atomic vector or list) represents a **variable** (column)
...
-   `dplyr` is a package with functions (a.k.a. "*verbs*") that form a consistent grammar of data transformation:
    ...
    -   `select` function selects columns/variables based on their names
    -   `pull` function pulls a column/variable based on its name
    -   `filter` function selects rows/observations based on their values
    -   `mutate` function adds new columns/variables that are functions of existing columns/variables
    -   `arrange` function changes the ordering of the rows/observations
    -   `summarize` function reduces multiple values down to a single value
