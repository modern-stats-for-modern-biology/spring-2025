...
-   Each R session has a **global environment** where user-defined objects reside; additional environments are created dynamically (e.g., when loading packages)
...
-   A **package** is a collection of objects (functions and data) and their documentation stored as local files in the computer’s disk
...
-   As of today, CRAN and Bioconductor feature [21,939](https://www.r-pkg.org/) and [2,289](https://www.bioconductor.org/packages/release/BiocViews.html) R packages, respectively!

-   Search for R packages using [CRAN](https://cloud.r-project.org/web/views/) and [Bioconductor](https://www.bioconductor.org/packages/release/BiocViews.html) "*task views*", as well as [rdrr.io](https://rdrr.io/) and [Rdocumentation](https://www.rdocumentation.org/) 

## R: Packages and environments

-   Use the `library` function to load a package from the computer's disk into memory as an environment, which is then attached to the so called **search list**
...
-   Use the `search` function to list all available environments
...
-   Use the `search` function to list the environments available after loading `coursekata`
...
-   Once loaded, the objects (functions and data) contained in a package are accessible within the homonymous environment in the current session
...
-   The `base` environment is always searched last, as it is the first package loaded when an R session starts
...
::: nonincremental
-   Use the `conflicts` function to check for conflicts between loaded packages
:::
...
-   Use the `c` function (short for "*combine*") to create a longer vector from shorter ones (order matters)
...
-   The **subset operator** `[]` select any number of elements from an input vector and return an output vector of the same type containing those elements
-   The **pull operators** `[[]]` and `$` select a single element from an input vector and return that element, possibly of a different type than the input vector
...
-   Use the `data.frame` or `tibble` function (preferred in the `tidyverse`) to create a data frame or tibble, respectively
