...
-   The goal of **deductive reasoning** is to determine whether the **conclusion is guaranteed to be true**, assuming the premises are true (i.e., the argument is valid)
...
-   The goal of **inductive reasoning** is to determine whether the **conclusion is likely to be true**, assuming the premises are true (i.e., the argument is strong)
...
-   Ontological interpretations
    ...
    -   Limiting relative frequency/proportion ("long-run" = ∞)
...
-   Epistemological interpretations
    ...
    -   [Measure of the strength of an inductive argument (objective Bayesian)]{.underline}

## From propositional logic to probability theory

-   **Propositional logic** is a mathematical language used to **represent knowledge** with propositions and **reason** about the **truth** of a conclusion given a set of premises (*deductive reasoning*)

-   **Probability theory** extends propositional logic to reason about the **plausibility** of a conclusion given a set of premises (*inductive reasoning*)
...
-   For example, given an argument with two atomic variables $A$ and $B$, the sample space of the argument consists of four possible outcomes
...
-   Event $X$ is the subset of the sample space where proposition $X$ is true
...
-   Event $X$ (subset of $\Omega$ where $X$ is true)
...
::: nonincremental
-   If $X$ is a tautology ($\top$, e.g., $A \lor \lnot A$), then event $X$ is the sample space $\Omega$
:::
...
::: nonincremental
-   If $X$ is a contradiction ($\bot$, e.g., $A \land \lnot A$), then event $X$ is the empty set $\emptyset$
:::
...
-   Next, we will implement this $P$ function in R to build an AI for deductive reasoning
...
-   We will assess the inductive strength of this argument following these steps:
    ...
    -   Build the truth table of this argument
        -   First, create data frame with all possible combinations of atomic variables
...
Create data frame with all possible combinations of atomic variables
...
-   In this example with **two blue balls** ($N_B = 2$) and **one white ball** ($N_W = 1$):

::: fragment
$$P(H \mid E) = \frac{2}{2 + 1} = 0.6667$$
:::

-   In general, for any number of blue balls ($N_B = b$) and white balls ($N_W = w$):

::: fragment
$$P(B \mid N_B = b, N_W = w) = \frac{b}{b + w}$$
:::

-   This formula reflects the logic of a [Bernoulli trial](https://en.wikipedia.org/wiki/Bernoulli_trial): the probability of success is the proportion of favorable outcomes

-   This logic is fundamental to inductive reasoning/probability theory
...
Three axioms/desiderata of probability theory ([Cox's theorem](https://en.wikipedia.org/wiki/Cox%27s_theorem))

1.  Probability of hypothesis $H$ given evidence $E$ can be represented by a real number

::: fragment
$$
P(H \mid E) \in \mathbb{R}
$$
:::

2.  Probability obeys the rules of propositional logic

3.  Probability obeys common-sense directional rules

 

[For more information:]{.fragment}

-   [Benji Metha - Probability as logical inference: A dramatic reading of E.T. Jaynes' textbook](https://youtu.be/zOeOgXbC5hE)

-   [Aubrey Clayton - One probability to rule them all?](https://youtu.be/HCG57e7Ogv8?si=2PY1EBoPfjLdS6yd)
...
## The golden rules of statistics
...
![](images/chainsaw_birthday_cake.png){height="550"}
:::
:::::

[[Everything wrong with statistics (and how to fix it)](https://youtu.be/be2wuOaglFY)]{.fragment}
