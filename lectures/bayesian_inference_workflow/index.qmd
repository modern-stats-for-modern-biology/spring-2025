...
-   Traditional probability (a.k.a. **data** or **sampling probability**):
:::
...
-   Traditional probability (a.k.a. **data** or **sampling probability**):
    ...
    -   $p(d \mid M)$ = probability of the data given a set of DGP model assumptions
:::

-   The problem of statistical inference

    -   **Inverse probability**: $P(M \mid d)$ = probability of $M$ given $d$
    -   [It is impossible to calculate the inverse probability!]{.underline}

-   The (more reasonable, limited) goal of statistical inference

    -   Let $M = \{M^*, \theta\}$, where $M^*$ (the functional form of $M$) is assumed and $\theta$ are the parameters of $M^*$ to be learned from $d$
    -   **Inferential probability**: $P(\theta \mid d, M^*)$ = probability of $\theta$ given $d$ and $M^*$
    -   $P(\theta \mid d)$ \[simpler, alternative notation\]
...
ss %>%
  ...
  gf_line(posterior ~ π, color = "orange", linewidth = 1) %>%
  gf_line(likelihood.norm ~ π, color = "green", linewidth = 1) %>%
  gf_labs(
    x = expression(pi),
    y = "P",
    title = "Prior (red), likelihood (green) and posterior (orange)")
```
