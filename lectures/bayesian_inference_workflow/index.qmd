...
$$p(x = 2, n = 3 \mid \pi) = \binom{3}{2}\pi^2(1-\pi)^1$$ ![](images/Slide48.png)
...
-   Our toy example modeled "blue vs. white/water vs. land" as a **binomial** process:

    -   We count how many times "blue" shows up in $n$ draws and infer the **unknown proportion** $\pi$

-   In **real biology**, the same idea applies to any “presence vs. absence” trait. For example:

    -   Researchers want to estimate the fraction of microglial cells in an AD mouse’s brain that exhibit disease-associated microglia (DAM) markers

    -   Each cell is either DAM ("blue") or non‐DAM ("white")

    -   Researchers gather a sample of microglia, count how many are DAM, and estimate $\pi$ of DAM cells in the AD mouse’s brain using statistical inference

## From globe tossing experiment to real science
