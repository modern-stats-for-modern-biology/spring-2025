...
$$
p(\omega \mid M_{N}) = \frac{1}{\sqrt{2\pi \sigma^2}} \exp \!\left( -\frac{\bigl(y - \mu \bigr)^2}{2\sigma^2} \right)
$$
...
$$
p(\omega \mid M_{\text{GLM}}) = \frac{1}{\sqrt{2\pi \sigma^2}} \exp \!\left( -\frac{\bigl(y - (\beta_0 + \beta_1 \cdot x) \bigr)^2}{2\sigma^2} \right)
$$
...
$$
p(x, y \mid M_{\text{GLM}}) = \frac{1}{\sqrt{2\pi \sigma^2}} \exp \!\left( -\frac{\bigl(y - (\beta_0 + \beta_1 \cdot x) \bigr)^2}{2\sigma^2} \right)
$$
...
-   Traditional probability (a.k.a. **data probability** or **likelihood**):
    ...
    -   $p(d \mid K_M)$ = probability of the data given a set of DGP model assumptions
:::

-   The problem of statistical inference

    -   **Inverse probability**: $P(K_M \mid d)$ = probability of $K_M$ given $d$
    -   [It is impossible to calculate the inverse probability!]{.underline}

-   The (more reasonable, limited) goal of statistical inference

    -   Let $K_M = \{M_{M^*}, \theta\}$, where $M^*$ (the functional form of $M$) is assumed and $\theta$ are the parameters of $M^*$ to be learned from $x$
