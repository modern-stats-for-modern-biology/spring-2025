...
-   Statistical inference is a logical (inductive) reasoning process using 1) *observables*/**data** and 2) a **model of the data generating process** (DGP) to infer *unobservables*:
...
-   The goal of **deductive reasoning** is to determine whether the **conclusion is guaranteed** to be true, assuming the premises are true (*truth-preserving, non-ampliative*, *certain*)
...
-   The goal of **inductive reasoning** is to determine whether the **conclusion is likely** to be true, assuming the premises are true (*not truth-preserving, ampliative*, *uncertain*)
...
-   Specifically, we will use:
    ...
    -   **R** (programming language) to translate a deductive or inductive argument written in the language of propositional logic and probability theory into a computer algorithm
...
-   The assignment of a truth value to an **atomic variable** (e.g., $A$ and $B$) is called **valuation**

-   The truth value of a **compound variable** is determined by the truth values of its atomic variables and the logical operators, according to the **rules of propositional logic**

-   A **truth table** is the **set of all possible valuations** for a set of atomic variables

-   A truth table for $n$ atomic variables consists of $2^n$ possible valuations

-   A truth table is also known as **universe** ($\Omega$), sample space, state space, etc.
...
Two propositions $X$ and $Y$ are **logically equivalent** ($X \equiv Y$) iff $X \iff Y$ is a tautology
:::
...
A proposition $X$ **logically entails** another proposition $Y$ ($X \models Y$) iff $X \implies Y$ is a tautology
:::
...
-   takes a deductive argument (one or more premises named `E` for evidence, and a conclusion named `H` for hypothesis) as input, and
...
Build truth table (set of all possible worlds)
...
Add premises, combine them as evidence, and add conclusion/hypothesis
...
Check that argument is valid $\quad E \models H$
...
#| output-location: default
is.valid <- function(..., H) {
  # Capture premises as quosures
  premises <- enquos(...)
  
  # Combine premises as evidence
  evidence_expr <- reduce(premises, ~ expr((!! .x) & (!! .y)))
  
  # Capture hypothesis as a quosure
  ...
  # Check that argument is valid: Hypothesis is true in all possible worlds where evidence is true
  tt %>%
    mutate(
      E = !!evidence_expr,
      H = !!hypothesis_expr
