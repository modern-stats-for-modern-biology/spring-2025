...
Statistical inference is logical reasoning using 1) **data** (*measurements of observable quantities*) and 2) a **model** of the data generating process (DGP) as evidence to infer *unobservable quantities*:

::: fragment
$$
\begin{array}{ll}
1. & \text{data (observations)} \\
2. & \text{process model (DGP)} \\
\hline
\therefore & \text{unobservable quantities} \\
\end{array}
$$
:::
...
-   In AI/ML, the set of premises is referred to as **knowledge base**, and the conclusion is referred to as **query**

-   In AI/ML, the term **background information/knowledge** is used to refer to implicit assumptions

## Propositions

-   A **proposition** (or statement) is a declarative sentence that can be either true or false, but not both at the same time
...
There are two types of propositional variables:

-   An **atomic variable** represents a single, indivisible proposition (e.g., "*The robot is blue.*")

-   A **compound variable** is formed by combining one or more atomic variables with **logical operators** (*e.g.,* "*The robot is blue **AND** has an antenna.*")

-   Logical operators are also known as logical connectives or Boolean operators
...
-   An argument is deductively valid iff the premises ($K = P1, P2, \ldots$) logically entail the conclusion ($C$), i.e., in every possible world where $K$ is true, $C$ is also true
...
::: fragment
$$
\boxed{V(\frac{K}{C}) = \begin{cases}
\text{True} & \text{if } K \models C \\
...
$$
\boxed{V(\text{C} \mid \text{K}) = \begin{cases}
\text{True} & \text{if } K \models C \\
\text{False} & \text{otherwise}
\end{cases}}
$$
...
-   Next, we will implement this $V$ function in R to build an AI for deductive reasoning
    ...
    -   **Input**: A deductive argument with one or more premises (`P1`, `P2`, …) and a conclusion (`C`)
...
-   We will determine the deductive validity of this argument in three steps:
    ...
    2.  Add premises ($K = P1, P2$) and conclusion ($C$) to the truth table
    3.  Check that in every possible world where $K$ is true, $C$ is also true
...
Add premises ($K = P1, P2$) and conclusion ($C$) to the truth table
...
Check that in every possible world where $K$ is true, $C$ is also true
...
```{r}
#| output-location: default
expand_grid(B1 = c(TRUE, FALSE), B2 = c(TRUE, FALSE), W  = c(TRUE, FALSE)) %>% mutate(P1 = B1 | B2 | W, P2 = !((B1 & B2) | (B1 & W) | (B2 & W)), K = P1 & P2, C  = B1 | B2) %>% filter(K) %>% pull(C) %>% all()
```
...
is.valid <- function(..., C) {
  ...
  # capture conclusion as a quosure
  C <- enquo(C)

  if (length(premises) == 0) {
    # if no premises are provided, set knowledge base K to TRUE (tautology)
    K <- expr(TRUE)
  } else {
    # else set knowledge base K to the conjunction of the premises
    K <- reduce(premises, ~ expr((!! .x) & (!! .y)))
  }
    
  # extract atomic variables from knowledge base and conclusion
  vars <- unique(c(all.vars(K), all.vars(C)))
  
  # build truth table
  tt <- expand_grid(!!!set_names(rep(list(c(TRUE, FALSE)), length(vars)), vars))
  
  # add knowledge base (K) and conclusion (C) to truth table
  # check that in every possible world where K is true, C is also true
  tt %>%
    mutate(K = !!K, C = !!C) %>%
    filter(K) %>%
...
P <- function(..., C, table = TRUE, flex = TRUE) {
  # capture premises as a list of quosures
  premises <- enquos(...)
  ...
  # extract atomic variables from premises and conclusion
  vars <- unique(c(unlist(lapply(premises, all.vars)), all.vars(C)))
  ...
  tt <- tt %>%
    mutate(
      K = if_all(starts_with("P"), identity),
      C = eval_tidy(C, data = tt)
    )
  ...
  # return P(C | K) if truth table output is not requested
  if (!table) {
    return(P)
  }
  ...
  tt.formatted <- flextable(tt) %>%
    ...
    bg(i = which(tt$K == TRUE), bg = highlight_color, part = "body")  # Highlight E = TRUE rows
...
#| echo: false
#| output-location: default
P(P1 = B1 | B2 | W, P2 = !((B1 & B2) | (B1 & W) | (B2 & W)), C = B1 | B2)
```
:::
