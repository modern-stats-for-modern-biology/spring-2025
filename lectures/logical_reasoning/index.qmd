...
::: fragment
A model is a function that relates known quantities to unknown quantities
:::
...
#| eval: false
unknown_quantities <- model(known_quantities)
```
:::
...
-   "*Known (or assumed) quantities*" → represents observed data, prior knowledge, or assumptions
-   "*A model*" → represents the formal structure that connects the known to the unknown
-   "*Infer unknown quantities*" → captures the goal of statistical inference, whether estimating missing values, latent variables, or future outcomes

A mathematical model is a function/mapping that relates/maps known quantities (data, assumptions, priors) to unknown quantities (parameters, predictions, latent variables, missing values)
...
#| output-location: default
#| eval: false
model(known_quantities) -> unknown_quantities
```
...
```{r}
#| output-location: default
X <- 1 # X: known quantity

Y <- NA # Y: unknown quantity

# model: Y = 10 - 7 * X
model <- function(X) 10 - 7 * X

# infer Y from X and model
Y <- model(X)
...
Statistical inference is inductive reasoning using **data** (*measured values of observable variables*) and a **model of the data-generating process (DGP)** to infer the *unmeasurable values of unobservable variables* (or *unmeasured values of observable variables*)
...
2. & \text{DGP model} \\
\hline
\therefore & \text{(see below)} \\
\end{array}
$$

-   Future observations (prediction/forecasting)
-   Unrecorded past observations or unobservable past events (retrodiction/backcasting)
...
-   In statistical inference, the conclusion is also known as the **hypothesis**, while the premises are collectively referred to as the **evidence**

-   In AI/ML, the conclusion is also known as the **query**, while the premises are collectively referred to as the **knowledge base** (or background information)
...
-   It represents an assertion or claim about a particular state of affairs in the world
    ...
    -   "*Unicorns poop ice cream!*" 🦄🍦 [is a proposition]{.fragment .green}
    -   "*2 + 2 = 5*" [is a proposition]{.fragment .green}
    -   "*X + 2 = 5*" [is not a proposition]{.fragment .red}

::: notes
-   "*Unicorns fly!* is a proposition, because it can be assigned a truth value

-   “a particular state of affairs in the world,” it’s not strictly about the real world but about any possible world where statements can be evaluated for their truth value
...
-   **Atomic propositions** are indivisible propositions (e.g., "*The robot is blue.*" or "*The robot has an antenna*")

-   **Compound propositions** are formed by combining one or more atomic propositions using **logical operators** (*e.g.,* "*The robot is blue **AND** has an antenna.*")

## Propositional logic: Propositional variables

-   A **propositional variable** is a symbol representing a proposition

-   A propositional variable holds a value (**truth value**) that is either **True** or **False**, but not both

-   Propositional variables are also known as logical or Boolean variables and are typically denoted by uppercase Roman letters
...
-   A **logical operator** (also called a logical connective or Boolean operator) is a symbol representing a function that combines one or more propositions (atomic or compound) to form a new compound proposition
...
-   The truth value of an **atomic variable** is assigned

-   The truth value of a **compound variable** is determined by the logical operators and the truth values of its atomic components, according to the **rules of propositional logic**

## Propositional logic: Truth tables

-   A **truth table** is a tabular representation of the set of all possible combinations of truth-value assignments for a given set of atomic variables, along with the truth values of compound variables formed by combining them using logical operators

-   A combination of truth-value assignments for a given set of atomic variables is also known as **valuation**

-   The set of all possible valuations for a given set of atomic variables is also known as **logical space**

-   The logical space for a set of $n$ atomic variables consists of $2^n$ possible valuations

-   Valuations are also known as **worlds** ($\omega$), outcomes, or states

-   The logical space is also known as the **universe** ($\Omega$), sample space, or state space

    -   The universe is the set of all possible worlds
...
expand_grid(
  A = c(TRUE, FALSE),
  B = c(TRUE, FALSE)) %>%
  mutate(`¬A` = !A, `¬B` = !B, `A ∧ B` = A & B, `A ∨ B` = A | B, `A ⇒ B` = A %=>% B, `A ⇔ B` = A %<=>% B)
```
:::
...
-   Recall: The goal of **deductive reasoning** is to determine the validity of an argument

::: notes
whether the **conclusion is guaranteed** to be true, assuming the premises are true
:::
...
-   Next, we will implement this $V$ function in R to build an AI for deductive reasoning
...
-   We will determine the deductive validity of this argument in three steps:

    1.  Build the truth table (the set of all possible worlds)
    2.  Add premises ($E = P1, P2$) and conclusion ($H$) to the truth table
    3.  Check that in every possible world where $E$ is true, $H$ is also true

## Proportional syllogism

Build the truth table (the set of all possible worlds)
...
Add premises ($E = P1, P2$) and conclusion ($H$) to the truth table
...
Check that in every possible world where $E$ is true, $H$ is also true
...
P <- function(..., H, table = TRUE, flex = TRUE) {
  ...
  # add premises to truth table
  for (i in seq_along(premises)) {
    tt[[paste0("P", i)]] <- eval_tidy(premises[[i]], data = tt)
  }
  
  # add conjunction of all premises (E) and H to truth table
  tt <- tt %>%
    mutate(
  ...
  # calculate P(H | E) by checking whether in every possible world where E is true, H is also true
  P <- tt %>%
    filter(E) %>%
    pull(H) %>%
    mean()
