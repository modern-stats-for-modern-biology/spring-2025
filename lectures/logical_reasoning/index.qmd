...
Y <- model(X)                    # infer Y = 3 from X = 1 and model Y = 10 - 7 * X
...
::: nonincremental
-   Statistical inference is inductive reasoning that uses **data** and a **model** of the data-generating process (DGP) to infer unmeasured or unobservable values of variables
:::
...
2. & \text{model (DGP)} \\
\hline
\therefore & \text{unknown quantities (see below)} \\
\end{array}
$$
...
## Propositional logic: Logical independence
