...
-   Statistical inference is an **inductive reasoning** process using *observables* (**data**) and a **model** of the data generating process (DGP) to infer *unobservables*

::: fragment
$$
\begin{array}{ll}
1. & \text{observables (data)} \\
...
::: notes
-   observables = data/observations/measurements/observed quantities
-   unobservables = unobserved or unobservable quantities
...
-   The goal of **deductive reasoning** is to determine whether the **conclusion is guaranteed** to be true, assuming the premises are true (*truth-preserving, non-ampliative*)
...
-   The goal of **inductive reasoning** is to determine whether the **conclusion is likely** to be true, assuming the premises are true (*not truth-preserving, ampliative*)
...
-   To achieve this, we need to translate deductive and inductive arguments from English to a language that the computer understands
:::
...
-   Specifically, we will use:
    ...
    -   **R** (a programming language) to translate deductive and inductive arguments written in the language of propositional logic and probability theory into a computer algorithm
...
-   The truth value of a compound variable is determined by the truth-value assignments of its atomic components according to the rules of propositional logic

-   The **truth table** is the **set of all possible truth-value assignments** of a set of atomic variables

-   The truth table is also known as **universe** ($\Omega$), sample space, state space, etc.

-   The truth-value assignments are also known as **worlds** ($\omega$), outcomes, states, etc.

::: notes
The truth table/logical space is the set of all possible truth-value assignments/valuations of a set of atomic variables
:::
...
::: fragment
Two propositions $X$ and $Y$ are **logically equivalent** ($X \equiv Y$) if and only if they have the same truth value in every possible world
:::
...
::: fragment
A proposition $X$ **entails** another proposition $Y$ ($X \models Y$) if and only if, in all possible worlds in which $X$ is true, $Y$ is also true.
:::
...
-   In the context of an argument, propositional variables are assumed to be true:\
    $A$ means $A$ is True   $\lnot A$ means $A$ is False

## The set of all possible worlds

-   The **universe** ($\Omega$) is the set of all possible worlds ($\omega_i$), i.e., all possible combinations of truth values of all atomic variables in an argument
