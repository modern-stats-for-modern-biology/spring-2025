...
-   Statistical inference is a logical (inductive) reasoning process using 1) **data** (observations/measurements of observable quantities) and 2) a **model of the data generating process** (DGP), to infer unobservable quantities:
...
-   Deductive validity is determined by checking whether the conclusion ($H$) necessarily follows from/is guaranteed by the premises ($E$)

-   Symbolically, this is expressed as $E \models H$, meaning $H$ is a logical implication of $E$
...
-   We will implement this AI in R as a function:

    -   **Input**: A deductive argument with one or more premises (`E` for evidence) and a conclusion (`H` for hypothesis)
...
\end{array}
$$
[We will approach this in three steps:]{.fragment}
...
2.  Add the premises and conclusion to the truth table

3.  Identify the possible worlds where all the premises (`E`) are true

4.  Check that the argument is deductively valid, i.e., that the conclusion (`H`) is true in each of those possible worlds ($E \models H$)
...
Add the premises and conclusion to the truth table
...
Check that the argument is deductively valid ($E \models H$)
