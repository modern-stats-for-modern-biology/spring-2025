...
-   Statistical inference is an *inductive reasoning* process where *known quantities* (**data**) and a **model** of the *data generating process* (DGP) are used to infer *unknown quantities*
...
![Raffaello Sanzio: The School of Athens (left, Plato; right, Aristotle)](images/Sanzio_01_Plato_Aristotle.jpg){fig-alt="https://en.wikipedia.org/wiki/The_School_of_Athens" fig-align="center" height="550"}
...
## AI: A computer algorithm for logical reasoning

::: nonincremental
-   Our goal in this series is to program an **artificial intelligence** (AI): a **computer algorithm** that is capable of **logical reasoning**
:::
...
-   To achieve this, we need to translate logical reasoning/arguments from English to a computer programming language
:::

-   Specifically, we will use:

    -   **Propositional logic**: a mathematical language used to formalize **deductive reasoning**

    -   **Probability theory**: a mathematical language used to formalize **inductive reasoning**

    -   **R**: a programming language used to translate propositional logic and probability theory into a computer algorithm
...
-   We will implement this AI in R as a function named `is.valid` that:
...
-   **Atomic variables** (e.g., $A$, $B$) each represent a single, indivisible proposition

::: fragment
| English                     | Logic | R                      |
|-----------------------------|-------|------------------------|
| *The robot is blue.*        | $A$   | `A <- TRUE` or `FALSE` |
| *The robot has an antenna.* | $B$   | `B <- TRUE` or `FALSE` |
:::

-   **Compound variables** are formed by combining atomic variables with *logical operators* (AND $\land$ `&`, OR $\lor$ `|`, NOT $\lnot$ `!`)
...
-   Atomic variables are assigned a `Logical` value (`TRUE` or `FALSE`, a.k.a. a *truth value*)

-   Compound variables derive their truth value from these atomic assignments and the logical operators, according to the rules of propositional logic
...
|-----------------|----------------|-------------|-------------|--------------|
| A               | $A$            | `A`         | `A == TRUE` |              |
| **NOT** A       | $\lnot A$      | `!A`        | `A != TRUE` | `A == FALSE` |
| A **AND** B     | $A \land B$    | `A & B`     |             | `all(A, B)`  |
| A **OR** B      | $A \lor B$     | `A | B`     | `!(A & B)`  | `any(A, B)`  |
...
## Logical operators: Biconditional

XXX

## Atomic vs. compound propositions

-   Propositions (a.k.a. propositional sentences) are made of propositional variables and logical connectives

-   Propositional variables (a.k.a. logical, Boolean, or binary variables) are typically represented by a capital letter
...
-   Propositional variables assume one of two values, typically represented by `TRUE` and `FALSE`
-   These values are implicit in the syntax of propositional logic, as we write $A$ to mean $A = \text{TRUE}$ and $\lnot A$ to mean $A = \text{FALSE}$

## Logical connectives and inference rules

-   **Logical connectives** (a.k.a. propositional, logical, or Boolean operators) are used to [form]{.underline} new propositions from existing ones

-   **Inference rules** are used to [derive]{.underline} the truth value of new propositions from those of existing ones

-   A proposition whose truth value is not derived from those of existing ones is called an **axiom**

-   Axioms serve as foundational premises/assumptions/evidence/knowledge base upon which the truth value of all other propositions is derived using inference rules

## Logical connectives and inference rules

::: fragment
**Negation (NOT)** $\quad \lnot A \quad A' \quad \bar{A} \quad A^{\complement}$
:::
...
::: fragment
**Conjunction/logical product (AND)** $\quad A \land B \quad A \cdot B \quad AB \quad A,B \quad A \cap B$
:::
...
-   $A \land \lnot A$ ["*the robot is blue AND it is NOT blue*"]{.fragment} [is an example of a **contradiction**:]{.fragment}<br/>[a proposition that is always false ($F$)]{.fragment}

-   $\lnot({A \land \lnot A)}$ is also known as the **law of non-contradiction**:<br/>[a proposition and its negation cannot both be true at the same time]{.fragment}

## Logical connectives and inference rules

::: fragment
**Disjunction/logical sum (OR)** $\quad A \lor B \quad A + B \quad A \cup B$
:::
...
-   $A \lor \lnot A$ ["*the robot is blue OR it is NOT blue*"]{.fragment} [is an example of a **tautology**:]{.fragment}<br/>[a proposition that is always true ($T$)]{.fragment}
-   $A \lor \lnot A$ is also known as the **law of excluded middle**:<br/>[either a proposition is true or its negation is true]{.fragment}

## Logical connectives and inference rules

::: fragment
**Conditional/Implication (IF-THEN, IMPLIES)** $\quad A \implies B \quad A \subseteq B$
:::
...
```{r}
#| output-location: slide
boolean_implies <- function(A, B) {
  return(1 - A + A * B) # return(!A | B)
}
```
:::
...
::: fragment
**Biconditional (IFF)** $\quad A \iff B \quad A = B$
:::
...
```{r}
#| output-location: slide
boolean_iff <- function(A, B) {
  return(A * B + (1 - A) * (1 - B)) # return(A == B)
}
```
:::
...
-   The **sample space** ($\Omega$) is the set of all possible worlds ($\omega_i$), i.e., all possible combinations of truth values of all propositional variables in an argument
