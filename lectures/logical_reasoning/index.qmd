...
$$
\textbf{IF}\ \text{(all men are mortal}\ \textbf{AND}\ \text{Socrates is a man)}\ \textbf{THEN}\ \text{Socrates is mortal}
$$
...
$$
\text{(all men are mortal}\ \textbf{AND}\ \text{Socrates is a man)}\ \textbf{IMPLIES}\ \text{Socrates is mortal}
$$

-   **Premises** are also known as **assumptions**, **facts**, or **rules**

-   In statistical inference, the conclusion is also known as the **hypothesis** (or claim), and the conjunction of the premises is known as the **evidence**

-   In AI/ML, the conclusion is also known as the **query**, and the conjunction of the premises is known as the **knowledge base** (or background information)
...
-   A **proposition** (or statement) is a declarative sentence that is either **True** or **False**, but not both
    ...
    -   "*X + 2 = 5*" 𝑓 [is not a proposition]{.fragment .red}
...
-   **Compound propositions** are formed by combining propositions using **logical operators**
    ...
    -   "*It's raining* **AND** *Gaborone is the capital of Botswana.*" 🌧🇧🇼

    -   "**IF** *2 + 2 = 5* **THEN** *Unicorns poop ice cream!*" 🧮🦄🍦
...
::: nonincremental
-   Our goal in this series of lectures is to program an **artificial intelligence (AI)**: A computer algorithm that is capable of logical reasoning
:::
...
-   The truth value of a compound proposition is **calculated** from the truth values of its atomic components, according to the **rules of propositional logic**
...
-   A **truth table** is a table listing all possible worlds in the universe along with the truth values of atomic and compound propositions

-   The propositions in the truth table define:
...
-   $A \land \lnot A$ "*The robot has an antenna **AND** it does **NOT** have an antenna.*" is an example of a **contradiction**, i.e., a proposition that is always false ($\bot$) and thus defines the $\emptyset$ event
...
-   $A \lor \lnot A$ "*The robot has an antenna **OR** it does **NOT** have an antenna.*" is an example of a **tautology**, i.e., a proposition that is always true ($\top$) and thus defines the $\Omega$ event
...
Two propositions  A  and  B  are logically dependent if knowing the truth value of  A  determines or restricts the truth value of  B
