...
-   Reasoning is usually formalized as an **argument**
...
-   In AI/ML, the **conjunction of premises** is known as the **knowledge base**, and the conclusion is referred to as the **query**

-   The term **background information** is also often used to refer to contextual knowledge or implicit assumptions
...
-   Therefore, we can formalize deductive reasoning as a function that takes an argument as input and returns true or false, indicating whether the argument is deductively valid

::: fragment
$$
\boxed{V(\frac{K}{C}) = \begin{cases} \text{True} & \text{if } K \models C \\ \text{False} & \text{otherwise} \end{cases}}
$$
:::
...
$$
\boxed{V(C \mid K) = \begin{cases} \text{True} & \text{if } K \models C \\ \text{False} & \text{otherwise} \end{cases}}
$$

## Artificial intelligence for deductive reasoning

-   Next, we will implement this $V$ function in R to build a simple AI for deductive reasoning
...
is.valid <- function(..., C) {
  ...
  # conjoin the premises into a knowledge base
  K_expr <- reduce(premises, ~ expr((!! .x) & (!! .y)))
  
  # capture the conclusion as a quosure
  C_expr <- enquo(C)
  
  # extract the atomic variables from the knowledge base and conclusion
  variables <- unique(c(
    all.vars(K_expr),
    all.vars(C_expr)
  ))
  ...
  # add the knowledge base (K) and conclusion (C) to the truth table
  # check that in every possible world where K is true, C is also true
  tt %>%
    mutate(
      K = !!K_expr,
...
P <- function(..., C, table = TRUE, flex = TRUE) {
  ...
  tt.formatted <- tt.formatted %>%
    add_footer_row(
      values = paste("P(C | K) =", round(P_value, 3)),
      colwidths = ncol(tt)
    ) %>%
...
Inductive generalization is a logical argument that draws a conclusion about a generalization based on a specific case
