...
-   Recall: The goal of **deductive reasoning** is to determine the deductive validity of an argument

::: notes
whether the **conclusion is guaranteed** to be true, assuming the premises are true
:::

-   An argument is deductively valid iff the premises ($E$) logically entail the conclusion ($H$), i.e., in every possible world where $E$ is true, $H$ is also true

::: fragment
$$E \models H$$
:::

-   Therefore, we can formalize deductive reasoning as a function that takes an argument as input and returns true or false, indicating whether it is deductively valid
...
-   We will determine the deductive validity of this argument in three steps:
    ...
    2.  Add premises ($E$) and conclusion ($H$) to the truth table
...
Add premises ($E$) and conclusion ($H$) to the truth table
...
is.valid <- function(..., H) {
  ...
  # add evidence (E) and hypothesis (H) to the truth table
  # check that in every possible world where E is true, H is also true
  tt %>%
    mutate(
      E = !!evidence_expr,
