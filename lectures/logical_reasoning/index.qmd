...
-   Observed data:
    ...
    -   Past observations
    -   Sample statistics (e.g., sample mean and variance)
    
-   Unobserved data:

    -   Prediction: future observations
    -   Estimation: population parameters (e.g., population mean and variance)
    -   Estimation: latent variables (e.g., factors in factor analysis)
...
$$
\begin{array}{ll}
1. & \text{All humans are mortal} \\
2. & \text{Socrates is a human} \\
\hline
...
$$
\begin{array}{ll}
1. & \text{The sun has risen every morning in recorded history} \\
\hline
\therefore & \text{The sun will rise tomorrow morning} \\
\end{array}
$$
:::
...
-   Our goal in this series is to program an **artificial intelligence (AI)**—a computer algorithm capable of **logical reasoning**

-   To achieve this, we need a **logical reasoning** and a **computer programming language**
...
-   This AI will be implemented in R as a function named `P` that
    ...
    -   takes one or more premises (**evidence**, named `E`) and a conclusion (**hypothesis**, named `H`) as input

    -   returns a `Numeric` value between `0` and `1` as output

        -   If `P(E, H) = 1`, the reasoning is deductively valid (`H` is necessarily true given `E`)

        -   If `0 < P(E, H) < 1`, the reasoning is deductively invalid (`H` is not necessarily true given `E`)

        -   If `P(E, H) = 0`, the reasoning is deductively invalid and contradictory (`H` is necessarily false given `E`)

## Propositional logic: Variables

| English                    | Logic | R           |              |
|----------------------------|-------|-------------|--------------|
| *the robot is blue*        | $A$   | `A <- TRUE` | `A <- FALSE` |
| *the robot has an antenna* | $B$   | `B <- TRUE` | `B <- FALSE` |
