...
-   This AI will be implemented in R as a function named `P` that:
    ...
    -   takes one or more premises (**evidence**, named `E`) and a conclusion (**hypothesis**, named `H`) as arguments `P(..., H)`

    -   returns a `Numeric` value between `0` and `1`

        -   If `P(..., H) = 1`, then the reasoning is deductively valid, i.e., `H` is necessarily true given `E`

        -   If `0 < P(E, H) < 1`, then the reasoning is deductively invalid

        -   If `P(E, H) = 0`, then the reasoning is deductively invalid and contradictory, i.e., `H` is necessarily false given `E`

## Propositional logic: Variables

| English                   | Logic | R          |             |
|---------------------------|-------|------------|-------------|
| *it's raining* (A)        | $A$   | `A = TRUE` | `A = FALSE` |
| *the streets are wet* (B) | $B$   | `B = TRUE` | `B = FALSE` |
...
| if A then B; A implies B | $A \implies B$ | `A %=>% B`  | `!A | B`   |             |
