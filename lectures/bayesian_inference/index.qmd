...
-   **Disjunction rule (OR)**: Sum rule
    ...
    -   Iff A and B are mutually exclusive, i.e., $P(A \land B) = 0$:
...
![](images/Slide50.png){fig-align="center"}

## Visual proof of Bayes' theorem

![](images/Slide51.png){fig-align="center"}
...
![](images/Slide54.png){fig-align="center"}

## Visual proof of Bayes' theorem

![](images/Slide55.png){fig-align="center"}
...
![](images/Slide59.png){fig-align="center"}

## Visual proof of Bayes' theorem

![](images/Slide60.png){fig-align="center"}
...
$$
P(H \mid E) = \frac{P(H) \cdot P(E \mid H)}{\sum_{i=1}^n P(P_i) \cdot P(E \mid P_i)}
$$
...
$$
P(H \mid E) = \frac{P(H) \cdot P(E \mid H)}{\int_{i=1}^\infty P(P_i) \cdot P(E \mid P_i)}
$$
...
-   This integral is often very difficult if not impossible to compute, and that's why Bayesian inference was relegated to simple cases until the advent of modern computers and [Markov Chain Monte Carlo](https://en.wikipedia.org/wiki/Markov_chain_Monte_Carlo) (MCMC) methods
...
::: fragment
$$P(X) = \sum_{\omega_i \in X} p(\omega_i)$$
:::
...
p_bg <- col_numeric(palette = c("transparent", "blue"), domain = c(0, 1))
...
ss %>%
  ...
  bold(part = "header") %>% 
  bg(j = ~ p, bg = p_bg) %>%
  style(i = ~ E == TRUE, j = "E", pr_t = fp_text(color = "white", bold = TRUE), pr_c = fp_cell(background.color = "forestgreen")) %>%
  style(i = ~ E == FALSE, j = "E", pr_t = fp_text(color = "white", bold = TRUE), pr_c = fp_cell(background.color = "firebrick"))
```
...
ss %>%
  ...
  mutate(
    ...
    H  = B1 | B2) %>%
  select(Ω, B1, B2, W, p, E, H) %>%
  mutate(p = ifelse(E, p / sum(p[E]), NA)) %>% # urn model
...
ss %>%
  ...
  style(i = ~ E == TRUE, j = "E", pr_t = fp_text(color = "white", bold = TRUE), pr_c = fp_cell(background.color = "forestgreen")) %>%
  style(i = ~ E == FALSE, j = "E", pr_t = fp_text(color = "white", bold = TRUE), pr_c = fp_cell(background.color = "firebrick")) %>% 
  style(i = ~ H == TRUE, j = "H", pr_t = fp_text(color = "white", bold = TRUE), pr_c = fp_cell(background.color = "forestgreen")) %>%
  style(i = ~ H == FALSE, j = "H", pr_t = fp_text(color = "white", bold = TRUE), pr_c = fp_cell(background.color = "firebrick"))
```
...
$\{ P_1, P_2, \ldots, P_n \} = \text{set of partitions/mutually exclusive and exhaustive events}$
