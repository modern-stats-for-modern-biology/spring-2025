...
:::: fragment
::: text-align-center
![](images/Slide137.png){fig-align="center" width="711"}
:::
::::
...
-   A statistical model is a **probability distribution** of **random variables** chosen to represent the data generating process (DGP)

-   Standard probability distributions (e.g., uniform, binomial, normal) encode different assumptions about the DGP; if one is a useful abstraction of the real process, that distribution becomes the statistical model

-   Standard probability distributions serve as ready-made "*data stories*"
...
-   Laplace's principle of indifference is a special case of Jayne's [principle of maximum entropy](https://en.wikipedia.org/wiki/Principle_of_maximum_entropy)
...
```{r}
#| output-location: default
ss %>% mutate(p = c(NA, NA, NA, NA)) %>% mutate(P1 = !A) # A is known or assumed to be `FALSE`
```
...
```{r}
#| output-location: default
ss %>% mutate(p = c(0, NA, NA, NA)) %>% mutate(P1 = !A) # A is known or assumed to be `FALSE`
```
...
```{r}
#| output-location: default
ss %>% mutate(p = c(0, 0, NA, NA)) %>% mutate(P1 = !A) # A is known or assumed to be `FALSE`
```
...
```{r}
#| output-location: default
ss %>% mutate(p = c(0, 0, 0.5, NA)) %>% mutate(P1 = !A) # A is known or assumed to be `FALSE`
```
...
```{r}
#| output-location: default
ss %>% mutate(p = c(0, 0, 0.5, 0.5)) %>% mutate(P1 = !A) # A is known or assumed to be `FALSE`
```
...
::: nonincremental
-   $p$ encodes our premises/assumptions/evidence/knowledge base
:::
...
-   Several other mathematical equations have been developed to capture different sets of premises/assumptions (e.g., binomial, Poisson, normal, and beta distributions)

-   Rather than enumerating each probability or specifying every premise/assumption, these standard probability distributions provide compact representations of the evidence/knowledge base underlying different "*data stories*
...
-   A random variable $X$ is a function that assigns a real number $x$ to each possible world/outcome $\omega$
...
-   Think of a random variable as a way to **measure** or **observe** something in each possible world
...
-   Alternatively, think of a random variable as a **question** about each possible world $\omega$, where the **answer is a real number** $x$
...
![](images/Slide121.png){height="550"}
...
-   Another way of thinking about a random variable is simply as a variable in R that can take on different values of different types
...
::: nonincremental
-   Without additional background information, the probability distribution is uniform
:::
...
```{r}
#| output-location: default
ss %>% gf_col(p ~ A, data = ., fill = ~ A) %>% gf_lims(y = c(0, 1))
```
...
```{r}
#| output-location: default
ss %>% gf_col(p ~ B, data = ., fill = ~ B) %>% gf_lims(y = c(0, 1))
```
...
```{r}
#| output-location: default
ss %>% gf_col(p ~ S, data = ., fill = ~ S) %>% gf_lims(y = c(0, 1))
```
...
```{r}
#| output-location: default
ss %>% gf_col(p ~ H, data = ., fill = ~ H) %>% gf_lims(y = c(0, 1)) %>% gf_refine(scale_fill_gradient(low = "lightblue", high = "darkblue"))
```
...
```{r}
#| output-location: default
ss %>% gf_col(p ~ W, data = ., fill = ~ W) %>% gf_lims(y = c(0, 1)) %>% gf_refine(scale_fill_gradient(low = "tan", high = "saddlebrown"))
```

## What is a random variable?

::: nonincremental
-   Adding premises/assumptions to the evidence/knowledge base changes the probability distribution
:::
...
ss <- ss %>%
  ...
  ungroup() %>% 
  mutate(p = ifelse(E, p / sum(p[E]), 0)) # update probability distribution using Bayes' theorem
