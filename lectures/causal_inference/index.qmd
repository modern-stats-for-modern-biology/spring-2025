...
Find all open backdoor paths between treatment and outcome
...
Use linear regression with covariates in adjustment set to estimate the causal effect of bed net use on malaria risk
...
#| output-location: default
#| fig-height: 8
gf_point(Y ~ X, data = tibble(X, Y, Z), color = "grey") %>% gf_lm(color = "darkgrey")
```
:::
...
#| output-location: default
#| fig-height: 8
gf_point(Y ~ X, data = tibble(X, Y, Z), color = ~ factor(Z)) %>% gf_lm() %>% gf_theme(legend.position="none")
```
:::
...
## Free and open-source causal inference software tools
